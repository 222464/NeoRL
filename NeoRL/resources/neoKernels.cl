	...
	int2 visibleSize, float2 hiddenToVisible, int radius, float reluLeak)
{
	...
	float state = sigmoid(state) * hiddenState;//relu(sum, reluLeak) * hiddenState;
	...
	float reluLeak)
{
	...
	float error = sum * state * (1.0f - state);//relud(state, reluLeak) * hiddenState;
