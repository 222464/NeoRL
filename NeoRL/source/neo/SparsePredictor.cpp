	...
	std::mt19937 &rng)
{
	...
	_learnDecoderWeightsRLKernel = cl::Kernel(program.getProgram(), "spLearnDecoderWeightsRL");
...
void SparsePredictor::learnRL(float tdError, sys::ComputeSystem &cs, const std::vector<cl::Image2D> &visibleStates,
	const std::vector<cl::Image2D> &feedBackStates, const std::vector<cl::Image2D> &addidionalErrors, float weightAlpha, float weightLambda, float biasAlpha, float activeRatio, float rmsDecay, float rmsEpsilon)
{
	// Start by clearing error summation buffer
	{
	...
	for (int vli = 0; vli < _visibleLayers.size(); vli++) {
		...
		if (vld._predict) {
			...
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._error);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, _hiddenStates[_back]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, feedBackStates[vli]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._predDecoderWeights[_back]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._predDecoderWeights[_front]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._feedBackDecoderWeights[_back]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._feedBackDecoderWeights[_front]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, _hiddenSize);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, _feedBackSizes[vli]);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._visibleToHidden);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vl._visibleToFeedBack);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vld._predDecodeRadius);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, vld._feedBackDecodeRadius);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, weightAlpha);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, weightLambda);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, rmsDecay);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, rmsEpsilon);
			_learnDecoderWeightsRLKernel.setArg(argIndex++, tdError);

			cs.getQueue().enqueueNDRangeKernel(_learnDecoderWeightsRLKernel, cl::NullRange, cl::NDRange(vld._size.x, vld._size.y));
