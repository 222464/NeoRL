	...
	do {
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::T)) {
			...
			for (int l = 0; l < layerDescs.size(); l++) {
				...
				cs.getQueue().enqueueReadImage(agent.getLayer(l)._sc.getHiddenActivations()[neo::_back], CL_TRUE, { 0, 0, 0 }, { static_cast<cl::size_type>(layerDescs[l]._size.x), static_cast<cl::size_type>(layerDescs[l]._size.y), 1 }, 0, 0, data.data());
